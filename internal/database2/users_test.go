...
func TestUsersCreate_Success(t *testing.T) {
	...
	o, err := db.UsersCreate(t.Context(), database2.UsersCreateParams{
		Name: "test",
		Params: database2.UserParams{
			TelegramID: "1234",
		},
	})
...
func TestUsersCreate_DuplicateName(t *testing.T) {
	...
	_, err := db.UsersCreate(t.Context(), database2.UsersCreateParams{
		Name: "user1",
	})
	assert.Error(t, err)
}
...
func TestUsersUpdate_Success(t *testing.T) {
	...
	updated, err := db.UsersUpdate(t.Context(), database2.UsersUpdateParams{
		ID:        o.ID,
		Name:      "user1_updated",
		Params:    params,
		IsAdmin:   o.IsAdmin,
...
func TestUsersGetByParams_Success(t *testing.T) {
	...
	o, err := db.UsersGetByParam(t.Context(), database2.UserTelegramID, "31337")
	assert.NoError(t, err)
	assert.Equal(t, "user1", o.Name)
}
...
func TestUsersGetByParams_NotExist(t *testing.T) {
	...
	_, err := db.UsersGetByParam(t.Context(), database2.UserTelegramID, "1")
	assert.Error(t, err)
	assert.EqualError(t, err, pgx.ErrNoRows.Error())
}
